...
public interface JadwalKuliahRepository extends JpaRepository<JadwalKuliah, Serializable> {
	@Query("SELECT jk.matkul.idMatkul AS idMatkul, jk.matkul.namaMatkul AS namaMatkul, jk.matkul.kdMatkul AS kodeMatkul, " +
			...
			"jk.ruangan.macAddress AS macAddress, jk.idJadwal AS idJadwal " +
	...
	@Query("SELECT jk.idJadwal " +
			...
			"ORDER BY jk.jam.jamKe ASC")
	List<Integer> getIdJadwal(Date tglKuliah, Time jam, String hari, String kdKelas, String kdMatkul);
	
	@Query("SELECT jk.idJadwal AS idJadwal, jk.jam.jamKe AS jamKe " +
			"FROM JadwalKuliah jk LEFT JOIN jk.jadwalPengganti jp ON jp.tglKuliah = ?1 " +
			"WHERE EXISTS(SELECT jk.idJadwal FROM JadwalKuliah jk LEFT JOIN jk.jadwalPengganti jp ON jp.tglKuliah = ?1 " +
			"WHERE jk.jam.jamMulai = ?2 AND jk.hari = ?3 AND jk.kelas.kdKelas = ?4 AND jp.jadwalKuliah.idJadwal IS NULL) " +
			"AND jk.hari = ?3 AND jk.kelas.kdKelas = ?4 AND jk.matkul.kdMatkul = ?5 AND jk.matkul.jenisMatkul = ?6 " +
			"AND jp.jadwalKuliah.idJadwal IS NULL " +
			"ORDER BY jk.jam.jamKe ASC")
	List<Map<String, Integer>> getIdJadwalJamKe(Date tglKuliah, Time jam, String hari, String kdKelas, String kdMatkul,
			boolean jenisMatkul);
	
	@Query("SELECT jk.idJadwal " +
			"FROM JadwalKuliah jk " +
			"WHERE jk.matkul.namaMatkul = ?1 AND jk.matkul.jenisMatkul = ?2 AND jk.kelas.kdKelas = ?3 AND jk.hari = ?4 " +
			"ORDER BY jk.jam.jamKe ASC")
	List<Integer> getIdJadwalByMatkul(String namaMatkul, boolean jenisMatkul, String kdKelas, String hari);
